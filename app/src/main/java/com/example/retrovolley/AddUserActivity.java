...
public class AddUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            ...
            if (typeConn.equalsIgnoreCase(getString(R.string.retrofit)))
                txtTitleLibrary.setText(getString(R.string.sendRetrofit));
            else txtTitleLibrary.setText(getString(R.string.sendvolley));
        }
    }

    public void submitByVolley(User user) {
        Gson gson = new Gson();
        String URL = "https://192.168.1.9/voller/User_Registration.php";

        ProgressDialog proDialog = new ProgressDialog(this);
        proDialog.setTitle(R.string.volley);
        proDialog.setMessage(getString(R.string.sdgSubmit));
        proDialog.show();
    ...
    public void submitByRetrofit(User user) {
        ...
        Retrofit.Builder builder = new Retrofit.Builder().baseUrl("https://192.168.1.9/volley/")
                .addConverterFactory(GsonConverterFactory.create());

        Retrofit retrofit = builder.build();
        MethodHTTP client = retrofit.create(MethodHTTP.class);
        Call<Request> call = client.senUser(user);
